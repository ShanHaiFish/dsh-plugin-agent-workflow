...
浏览器端 React 插件（`src/client/`，宿主端无行为），适配 dsh@0.1.0-rc.8。
...
| 安装到本机 dsh | `npx --yes @deepseek-ai/dsh@0.1.0-rc.8 plugin --profile web add ./dsh-plugin-agent-workflow-<version>.tgz --workspace-root` |
...
tests/
  *.client.spec.ts(x)     vitest 单测（include: tests/**/*.spec.ts(x)）
  mocks/                  client-runtime / ui-primitives 替身（vitest 别名指向）
...
- 适配面固定 dsh@0.1.0-rc.8：全部 `@deepseek-ai/*` 依赖钉在 `0.1.0-rc.8`（`package.json`）；升级 DSH 需同步升级本插件依赖（`README.md`「兼容版本」）。
- 发布用 `v0.1.x` 标签，GitHub 安装固定版本形如 `github:<owner>/<repo>#v0.1.1`（`README.md`「从 GitHub 安装」）。
...
| 测试（涉及注入服务） | `tests/mocks/client-runtime.ts`、`tests/mocks/ui-primitives.tsx`（配套 `vitest.config.ts` 的别名） |
...
- 新增运行时依赖（值导入）必须进 `tsdown.config.ts` 的 `onlyBundle` 白名单，否则构建失败；`CLIENT_EXTERNALS`（react、cordis、dsh-client-runtime/client、dsh-client-ui-primitives）保持外部 import，不打包。
- CSS 只能用 `.module.css`：由 tsdown 的 cssModulePlugin 内联为运行时注入的 `<style data-plugin-css>`（自动去重）；不要引入普通 CSS 或 CSS 框架。
- 测试禁止直接 import 真实的 `@deepseek-ai/dsh-client-runtime/client` 与 `@deepseek-ai/dsh-client-ui-primitives`——`vitest.config.ts` 已把它们别名到 `tests/mocks/`。
